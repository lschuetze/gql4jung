...
public class JarReader {
	...
	public synchronized DirectedGraph<TypeNode, TypeReference> readGraph()	throws GraphIOException {
		...
		if (jars.size()==0) {
			...
			return new DirectedSparseGraph<TypeNode, TypeReference>();
		}
		...
		final DirectedGraph<TypeNode, TypeReference> graph = new DirectedSparseGraph<TypeNode, TypeReference> ();
	...
	private void addVertex(DirectedGraph<TypeNode, TypeReference> graph,Classfile classfile,int id,Map<String,TypeNode> vertices,Map<Classfile,String> containerMapping) {
	...
	private void addEdge(DirectedGraph<TypeNode, TypeReference> graph, Node start,Node end, boolean isUses, int counter, Map<String, TypeNode> vertices) {
		...
		if (target!=null) { // this is possible - reference to external class
			...
			if (addEdge && this.removeDuplicateEdges) {
				for (TypeReference e:graph.getOutEdges(source)) {
					// TODO, FIXME
			...
			if (addEdge) {
				TypeReference edge = new TypeReference();

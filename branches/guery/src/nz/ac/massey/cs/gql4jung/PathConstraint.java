...
public class PathConstraint<V extends Vertex<E>,E extends Edge<V>> implements Constraint {
	...
	public Iterator<Path<V,E>> getPossibleSources(final DirectedGraph<V,E> g,final V target) {
		return PathFinder.findLinks(g,target,this.minLength,this.maxLength, false,filter,computeAll);
	}
	public Iterator<Path<V,E>>  getPossibleTargets(final DirectedGraph<V,E> g, final V source){
		return PathFinder.findLinks(g,source,this.minLength,this.maxLength, true, filter,computeAll);
	}

	public Iterator<Path<V,E>> check(final DirectedGraph<V,E> g, final V source, final V target){
		if (this.computeAll) {
			Iterator<Path<V,E>> allOutgoing = PathFinder.findLinks(g,source,this.minLength,this.maxLength, true, filter,computeAll);
			...
			Path<V,E> path = ConstraintedShortestPathFinder.findLink(g,source,target,minLength,maxLength,filter);
			if (path==null) {
